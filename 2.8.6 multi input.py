if __name__ == "__main__":
    ...
    while new_values != ' ' and new_values != '[':
        ...
        if ';' in new_values:
            ...
            continue
        if '[' in new_values:
            list_values = new_values.strip('][').split(', ')
            break
        rps_values.append(int(new_values))
    # TODO проверяем какой из двух списков заполнен
    if not rps_values:
        rps_with_two_values = list(map(int, list_values))
        print(rps_with_two_values)
        print(type(rps_with_two_values))
        rps_first_value = rps_with_two_values[:1]
        rps_second_value = rps_with_two_values[-1:]
    else:
